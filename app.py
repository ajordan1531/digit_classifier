...
with col_draw:
    if st.button("🗑️ Clear"):
        st.session_state.canvas_key += 1  # new key -> fresh, empty canvas
        st.rerun()
        ...
        if img is not None:
            ...
            if hasattr(model, "predict_proba"):
                try:
                    ...
                    # e.g. SVC(probability=False): method exists but isn't usable
                    probs = None
            ...
            if probs is not None:
                ...
                st.caption("Confidence scores unavailable (model trained without probability=True)")
